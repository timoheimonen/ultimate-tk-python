...
- Continued Phase 15 evaluation observability: extended `tools/ppo_eval.py` with aggregate terminal-reason distribution output plus optional `--summary-json-out` artifact export, updated PPO CLI help smoke coverage (`tests/unit/test_ppo_tools_cli.py`), and validated end-to-end with short train/eval smoke (`tools/ppo_train.py --run-name phase15_proto_smoke ...`, `tools/ppo_eval.py --summary-json-out ...`) and focused unit matrix (`27 passed`).
...
- Completed face-engagement reward redesign in `src/ultimatetk/ai/reward.py`: replaced 360-degree `look_at_enemy_reward` with front-sector-only `face_enemy_reward` (sectors 0/1/31) and additive `aim_center_reward` (sector 0); added `alignment_improvement_reward` (delta-based, skips first sighting), `valid_shot_reward` (shot + enemy in front), `frontier_tile_reward` (Chebyshev distance from spawn), and `inactivity_cost` (resets on exploration, combat progress, or valid shot); removed `strafing_reward`, `idle_cost`, `bad_shoot_cost`, and `visible_no_hit_cost`; renamed `stationary_ticks` → `inactivity_ticks` in `RewardStep` and `gym_env.py`; rewrote `tests/unit/test_gym_reward.py` (43 tests) with `_test_cfg()` helper for isolated reward testing; updated `docs/notes/ai_gym_reward_improvements.md` to authoritative current state; validated with full AI test suite (`pytest tests/unit/test_gym_reward.py tests/unit/test_gym_env.py` -> `54 passed`) and random-policy smoke.
