...
- Main menu is currently a scaffold (`autostart_gameplay` shortcut), not an interactive flow.
...
1. Main menu interaction and transition wiring (pending)
...
- [ ] Confirm initial Phase 6 target UX path (menu start + quit + level advance).
- [ ] Define first minimal completion trigger for level progression path.
- [ ] Implement Workstream 1 with unit scene-flow coverage.
