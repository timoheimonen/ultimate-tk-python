...
- Section 4 input/player control includes movement/collision/camera/weapon switching, shoot-reload cadence, and first-pass bi-directional combat (enemy spawn/aim/move/shoot + travel-time enemy projectiles + destructible/collectible crates with rewards + first-pass weapon ammo consumption/caps + ammo runtime telemetry snapshots for current weapon/full pools + first-pass shop trading helpers for ammo/weapons/shield/target + first-pass in-game shop flow with selection/transactions + first-pass visual shop overlay panel with legacy item naming labels and blocked-reason feedback + first-pass in-game HUD overlay for weapon/ammo/health/cash/shield/target + first-pass player mine/C4 deploy-arming-detonation flow + wall-aware explosive blast rays for mine/C4 detonation damage with partial-cover falloff weighting, corner/corridor-oriented obstruction tuning, scripted obstruction scenarios, and finer ray-trace stepping to damp tile-edge grazing leaks + wall-aware enemy grenade/projectile splash coverage for blocked versus partial blast lanes + tighter enemy LOS corner-graze sampling so wall-edge peeks no longer trigger false shots, with scripted open-vs-blocked coverage + tighter enemy direct-shot sampling to reduce corner-graze pellet leakage, with scripted direct-shot corner-graze coverage + tighter enemy projectile sub-step sampling to reduce corner-graze projectile leakage, with scripted projectile corner-graze coverage + runtime/HUD telemetry for active player explosives + player/enemy damage tracking + game-over flow).
